...
int rechercheEtudiant(char matriculeEtu[20]){
    ...
    if(fichier != NULL){
     ...
     printf("Nom : %s\t Prenom : %s\t Date et lieu de naissance : %d %s %d a %s\n",
            etud.nom, etud.prenom, etud.dateDeNaissance.jour, etud.dateDeNaissance.mois,
             etud.dateDeNaissance.annee, etud.lieu);
    printf("Matricule : %s\t Sexe : %s \n", etud.matricule, etud.sexe);
